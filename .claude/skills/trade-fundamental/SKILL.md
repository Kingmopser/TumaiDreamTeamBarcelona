...
1. **Direct invocation** — User runs `/trade fundamental <TICKER>` or `/trade fundamental <TICKER> <DATE>`. You must gather all data yourself via WebSearch.
...
**Date handling:** If a date is provided (e.g. `2025-04-15` or `"as of April 2025"`), treat it as the **data cutoff date** — all data gathered and analysis performed must reflect the company's state as of that date. Use only filings, reports, and prices available on or before that date. If no date is provided, use the most recent data available.

Set `ANALYSIS_DATE` from the input (default: today). Use `ANALYSIS_DATE` in every search query below by substituting the placeholder `<DATE_CONTEXT>`.
...
Query: `"<TICKER> stock valuation P/E P/S P/B PEG EV/EBITDA <DATE_CONTEXT>"`
Gather:
- Trailing P/E ratio
- Forward P/E ratio
...
Query: `"<TICKER> revenue earnings growth rate guidance <DATE_CONTEXT>"`
Gather:
...
Query: `"<TICKER> profit margins ROE ROIC gross operating net margin <DATE_CONTEXT>"`
Gather:
...
Query: `"<TICKER> balance sheet debt equity free cash flow ratio <DATE_CONTEXT>"`
Gather:
...
Query: `"<TICKER> competitive advantage moat market position competitors <DATE_CONTEXT>"`
Gather:
...
Query: `"<TICKER> insider ownership CEO management capital allocation <DATE_CONTEXT>"`
Gather:
...
> Data cutoff: <ANALYSIS_DATE> (omit if using latest data)
...
7. If `ANALYSIS_DATE` is set, ONLY use data available on or before that date. Explicitly note the data cutoff at the top of the report (e.g. `> Data cutoff: April 15, 2025`). If no date is provided, use the most recent data available and note when data was last updated.
